...
if [[ $# -lt 3 || $# -gt 3 ]]
then
   printf "\nUsage: $0 <server name> <IP address> <name of intermediate-CA>\n"
   printf "Ex.: ./gen_cert.sh server1 172.31.11.10 int\n\n"
   exit -1
fi
...
-addext "subjectAltName = DNS:localhost,DNS:*.localhost,DNS:$1.com,DNS:$DOMAIN,IP:127.0.0.1,IP:$2" \
...
-extfile - <<<"subjectAltName = DNS:localhost,DNS:*.localhost,DNS:$1.$DOMAIN,IP:127.0.0.1,IP:$2
